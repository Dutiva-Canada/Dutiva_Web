...
| `/app/documents` → `studio · templates/:tid · generate/:templateId · sign/:envelopeId · :docId`                                                                                           | Document Library — Repository (index, `:docId`), Studio (`studio` and its `templates/generate` sub-flow), and the embedded signing page `sign/:envelopeId` share one sidebar entry |
