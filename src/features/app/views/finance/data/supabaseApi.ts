...
export async function loadFinanceStateFromSupabase(orgId: string): Promise<FinanceWorkspaceState> {
  ...
  const [
    ...
    selectAll(TABLES.deals, mapDeal),
    selectAll(TABLES.commitments, mapCommitment),
    selectAll(TABLES.capitalCalls, mapCapitalCall),
    selectAll(TABLES.cashSweeps, mapCashSweep, 'scheduled_date'),
    selectAll(TABLES.documentLinks, mapDocumentLink),
