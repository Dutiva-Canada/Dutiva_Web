...
/**
 * Conversation / thread memory in production — fact list only.
 * Chat-recall transcripts (RECALL_TURNS) stay demo-only.
 */
...
export function ChatRecallProductionView() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadFailed(false)
    try {
      setFacts(await listFactsByEntity(organizationId, 'thread', threadId))
    } catch {
      setFacts([])
      setLoadFailed(true)
    }
