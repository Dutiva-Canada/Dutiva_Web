...
function TourStopPreviewDialog({
  ...
  return createPortal(
    <div className="surface-marketing dutiva-surface text-text fixed inset-0 z-300 pt-[env(safe-area-inset-top)]">
      ...
      >
        <div className="pointer-events-auto flex max-h-[min(88vh,640px)] w-[min(480px,100%)] flex-col overflow-hidden rounded-[16px] border border-border bg-bg-elevated shadow-modal">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
            <p className="m-0 text-sm leading-[1.55] text-text-2">{x(stop.blurb)}</p>
            <p className="mt-3 text-xs leading-normal text-text-faint">{lt('landing_ws_demo_preview_note')}</p>
          </div>
