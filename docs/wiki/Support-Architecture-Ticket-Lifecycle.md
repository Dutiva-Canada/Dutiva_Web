...
The `renderSupportEmail()` function in `src/features/support/email/templates.ts` is the tested source of truth for all 14 notification email templates. The `support-notify` edge function mirrors this as `renderNotificationEmail()`.

Templates follow strict rules:
- Subjects carry **only** the public reference — never description or PII — [src/features/support/email/templates.ts:8-9]()
- Bodies link to the authenticated ticket URL — [src/features/support/email/templates.ts:36-37]()
- All templates include the legal disclaimer and sensitive-info warning — [src/features/support/email/templates.ts:56-69]()
