...
The shell adds `PublicDemoBanner` (sample data, read-only) and `DemoTourRail` (five guided stops: Home, Advisor, Document Studio, Workflows, Cases). Landing-page mini-simulations link into `/demo/advisor` and related routes. Subpaths rewrite to `app.html` on Vercel; `/demo` and `/fr/demo` index pages are prerendered for SEO (`demoWorkspace` route).
